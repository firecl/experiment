...
function convertNodeToTurboElement(node: any) {
...
function convertEdgeToTurboElement(edge: any) {
  const {
    id,
    type,
    sourceNodeId,
...
export function toTurboData(data: { nodes: any[]; edges: any[] }) {
  const nodeMap = new Map()
  const turboData = {
    flowElementList: <any>[],
  }
  data.nodes.forEach((node: { id: any }) => {
...
function convertFlowElementToEdge(element: {
  ...
  Object.keys(element.properties).forEach(property => {
    if (excludeProperties.indexOf(property) === -1) {
      ;(edge.properties as any)[property] = element.properties[property]
    }
  })
  return edge
}

function convertFlowElementToNode(element: { properties: any; key?: any }) {
  ...
  Object.keys(element.properties).forEach(property => {
    if (excludeProperties.indexOf(property) === -1) {
      ;(node.properties as any)[property] = element.properties[property]
    }
  })
  return node
}
...
export function toLogicflowData(data: { flowElementList: any }) {
  ...
  list &&
    list.length > 0 &&
    list.forEach(
      (element: { properties: any; incoming?: any; outgoing?: any; key?: any; type: number }) => {
